...
PYBIND11_MODULE(_threedimensions_core, m) {
    ...
    py::class_<Core::Mesh>(m, "Mesh")
        ...
        // Topology
        .def("merge_duplicate_vertices", &Core::Mesh::mergeDuplicateVertices, py::arg("tolerance") = 1e-6f)
        // IO
    ...
    // Modifiers (functional helpers)
    m.def("apply_subdivision", &Modifiers::ApplySubdivision, "Apply subdivision to a mesh", py::arg("mesh"), py::arg("levels") = 1);
    m.def("apply_mirror", &Modifiers::ApplyMirror, "Apply mirror to a mesh", py::arg("mesh"), py::arg("axis"));
    m.def("apply_array", &Modifiers::ApplyArray, "Apply array to a mesh", py::arg("mesh"), py::arg("count"), py::arg("offset"));
    m.def("apply_weld", &Modifiers::ApplyWeld, "Weld duplicate vertices", py::arg("mesh"), py::arg("tolerance") = 1e-6f);

    // Sculpt (functional helpers)
    m.def("sculpt_inflate", &Sculpt::Inflate, "Inflate sculpt brush", py::arg("mesh"), py::arg("center"), py::arg("radius"), py::arg("strength"));
    m.def("sculpt_smooth", &Sculpt::Smooth, "Smooth sculpt brush", py::arg("mesh"), py::arg("center"), py::arg("radius"), py::arg("strength"));

    // Topology
    m.def("topology_laplacian_smooth", &Topology::LaplacianSmooth, "Laplacian smooth whole mesh", py::arg("mesh"), py::arg("iterations") = 1, py::arg("strength") = 0.5f);
}
